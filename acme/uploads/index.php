...
//call the buildNav() functions to create the $navList variable for the main menu views
$navList = navigation($categories);//buildName()
...
/* * ****************************************************
 * Image Upload Control Structure Switch Statement
 * **************************************************** */
switch ($action) {
 case 'upload':
// Store the incoming product id
  ...
  if ($imageCheck) {
   $message = '<p class="alert">An image by that name already exists.</p>';
  } elseif (empty($invId) || empty($imgName)) {
   $message = '<p class="alert">You must select a product and image file for the product.</p>';
  } else {
   ...
   if ($result) {
    $message = '<p class="alert">The upload succeeded.</p>';
   } else {
    $message = '<p class="alert">Sorry, the upload failed.</p>';
   }
  }
...
if ($remove) {
 $message = "<p class='alert'>$filename was successfully deleted.</p>";
} else {
...
if (count($imageArray)) {
 ...
 $imageDisplay = '<p class="alert">Sorry, no images could be found.</p>';
}
